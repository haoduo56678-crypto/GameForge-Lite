...
- 386 个基础语义概念、4,500 多个中英文直接表达，以及约 80,000 种可组合表达
- 新增 4,017 条高优先级词语与短语扩展，运行时形成 6,402 个有效匹配形式
...
- **基础语义层**：386 个概念、4,500 多个直接别名，并通过句式组合支持约 80,000 种表达。
- **词语与参数扩展层**：4,017 条源词语与短语，形成 6,402 个有效匹配形式；额外覆盖 100 个高优先级概念、183 个 Minecraft 物品别名、70 个生物与 Boss 别名、16 类数值参数和 15 组颜色表达。

扩展层只增强识别、同义词、繁简体、英文变体、中文数字和参数提取，不会替换原有生成器，也不会把未实现的大型玩法伪装成已经完成的功能。
...
词库负责“听懂并标准化”这些概念；某个概念是否已经能生成完整可玩的内容，仍取决于对应模板和生成器是否已经实现。未实现的世界、维度或大型系统会先被保存为概念草案，而不是被错误当成武器或普通方块。
